(ns architecture.advisor
  "ArchitectureAdvisor — proposes a design operation (draft design concept,
  prepare specification, flag code compliance concern, request client review)
  ...
  of project provenance or compliance-certification responsibility;
  ...
  A proposal is a map:
    {:op :draft-design-concept|:prepare-specification|:flag-code-compliance-issue|:request-client-review
...
(def ^:private system-prompt
  "You are an architecture design advisor. Given a design operation
   request, propose an :op, an honest :confidence (0.0-1.0), and a
   :stake (:low/:medium/:high). Never fabricate confidence you don't
   have. You never issue stamped designs or certify code compliance —
   those remain the licensed architect's exclusive responsibility.")
